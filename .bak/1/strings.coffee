...
do ->
	...
	typesPredicates.StringOrNumber= (value) -> typesPredicates.isString(value) or typesPredicates.isNumber(value)
...
class _ extends Types
	...
	@limitNumber= ( nr, [min, max] ) ->
		return nr if not Types.allNumber nr, min, max
		return max if nr > max
		return min if nr < min
		return nr
...
class Strings extends Chars
	...
	@random: ( amount= 1, charSet ) ->
		return '' if not _.stringOrNumber(amount) or (amount < 1)
		string= ''
